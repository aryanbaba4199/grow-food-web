...
export const generatePDF = ({data}) => {
  ...
  const props = {
    ...
    invoice: {
      ...
      additionalRows: [
        { col1: 'Total:', col2: products.reduce((sum, p) => sum + (p.price * p.quantity), 0).toFixed(2), col3: 'INR', style: { fontSize: 14 } },
        { col1: 'GST:', col2: '18', col3: '%', style: { fontSize: 10 } },
        { col1: 'SubTotal:', col2: (products.reduce((sum, p) => sum + (p.price * p.quantity), 0) * 1.18).toFixed(2), col3: 'INR', style: { fontSize: 10 } }
      ],
...
export const getUserGeoLocation = async () => {
  if (!navigator.geolocation) {
    return { response: false, data: 'Geolocation is not supported by this browser.' };
  }
      ...
      async (position) => {
        try {
          ...
          if (data.results.length > 0) {
            ...
            resolve({ response: false, data: 'No location found for the given coordinates.' });
          }
        } catch (error) {
